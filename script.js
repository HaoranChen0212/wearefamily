...
let haters = [];
let goodPairs = [];
let totalAppearances = 11; // 每个选手的参赛次数
...
// function generateMatchups(players) {
//     players = players.slice(); // Copy players array
//     players.forEach(p => {
//         p.appearances = 0;
//         p.priority = 0;
//     });
...
//     function matchInHistory(history, pair) {
//         let count = 0;
//         for (const [p1, p2] of pair) {
//             for (const record of history) {
//                 if ((record[0] === p1.name && record[1] === p2.name) || (record[0] === p2.name && record[1] === p1.name)) {
//                     count++;
//                 }
//             }
//         }
...
//     function tryGenerate(currentPlayers, currentMatches, currentHistory) {
//         if (players.every(p => p.appearances >= totalAppearances)) {
//             matches.push(...currentMatches);
//             return true;
//         }

//         currentPlayers.sort((a, b) => a.appearances - b.appearances || b.priority - a.priority);

//         const availablePlayers = currentPlayers.filter(p => p.appearances < totalAppearances);
...
//             for (let j = i + 1; j < selectedPlayers.length; j++) {
//                 const team1 = [selectedPlayers[i], selectedPlayers[j]];
//                 if (!validGender(team1)) continue;
//                 const remaining1 = selectedPlayers.filter(p => !team1.includes(p));
//                 for (let k = 0; k < remaining1.length; k++) {
//                     for (let l = k + 1; l < remaining1.length; l++) {
//                         const team2 = [remaining1[k], remaining1[l]];
//                         if (!validGender(team2)) continue;
//                         const remaining2 = remaining1.filter(p => !team2.includes(p));
//                         for (let m = 0; m < remaining2.length; m++) {
//                             for (let n = m + 1; n < remaining2.length; n++) {
//                                 const team3 = [remaining2[m], remaining2[n]];
//                                 if (!validGender(team3)) continue;
//                                 const team4 = remaining2.filter(p => !team3.includes(p));
//                                 if (!validGender(team4)) continue;
//                                 validMatches.push([[team1, team2], [team3, team4]]);
//                             }
//                         }
//                     }
//                 }
...
//         validMatches.sort((a, b) => matchInHistory(currentHistory, a) - matchInHistory(currentHistory, b));

//         for (const match of validMatches) {
//             const [[team1, team2], [team3, team4]] = match;

//             const involved = [...team1, ...team2, ...team3, ...team4];
//             let canProceed = involved.every(p => p.appearances < totalAppearances);
...
//                 if (!involved.includes(p)) {
//                     p.priority += 1;
//                 }
//             });

//             const newHistory = currentHistory.slice();
//             newHistory.push([team1[0].name, team1[1].name]);
//             newHistory.push([team2[0].name, team2[1].name]);
//             newHistory.push([team3[0].name, team3[1].name]);
//             newHistory.push([team4[0].name, team4[1].name]);

//             const newMatches = currentMatches.concat([
//                 {
//                     round: index + 1,
//                     match: 2 * (index + 1) - 1,
//                     team1: { players: team1 },
...
//             if (tryGenerate(currentPlayers, newMatches, newHistory)) {
//                 return true;
//             }
